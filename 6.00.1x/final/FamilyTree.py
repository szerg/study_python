...
class Family(object):
    ...
    def TracePath(self,node):
        if not node.get_parent():
            ...
            return [node] + self.TracePath(node.get_parent())
    ...
    def cousin(self, a, b):
        ...
        ## YOUR CODE HERE ####
        # gasesc calea catre radacina a fiecaruia
        trace_a = self.TracePath(self.names_to_nodes[a])
        trace_b = self.TracePath(self.names_to_nodes[b])
        if a == b:
            # acelasi nod nu e cousin si distanta pana la stramosul comun e aceeasi
            return -1,0
        elif self.is_child(a,b) or self.is_child(b,a):
            # in caz de tata-fiu distanta absoluta fata de acelasi stramos difera cu 1 si nu sunt cousins
            return -1,1
        else:
            # gaseste stramosul comun si returneaza minimul distantelor pana acolo fara radacina (acel -1) si distanta absoluta pt degrees removed
            for i,iv in enumerate(trace_a):
                for j,jv in enumerate(trace_b):
                    if iv == jv:
                        return min(i,j)-1,abs(i-j) 
